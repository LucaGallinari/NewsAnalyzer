...
def FarooPerformRequest(freq, jsonret):
	"""
...
class Faroo:
	...
	# Perform query
	def query(self, json=False, q=None):
		"""Perform a query"""
		self.param('q', q)
		...
		try:
			if json:
				return FarooPerformRequest(freq, True)
			else:
				return FarooResponse(FarooPerformRequest(freq, False))
